...
const chapters = ['The question', 'Build a plan', 'Reveal the conflict', 'Let Z3 choose', 'Explore']
...
function Tour({ scenario }: { scenario: Scenario }) {
  ...
  function solve(label: string, scope: Run['scope'], overrides: Partial<SolveRequest> = {}) {
    const request = { ...scenario.defaults, ...overrides, timeoutMs: 15000 }
    if (client.current?.solve(request)) { setComparison(undefined); setRun({ label, request, scope }) }
  }
  function compareSolvers() {
    const request = { ...scenario.defaults, ...(comparisonCase === 'impossible' ? { maxLockers: 7 } : {}), timeoutMs: 15000 }
    const label = comparisonCase === 'impossible' ? 'Can seven lockers work? (impossible)' : 'Can any network fit? (feasible)'
  ...
  function toggleDepot(id: string) {
    ...
    setSelectedDepots(current => current.includes(id) ? current.filter(value => value !== id) : current.length < 2 ? [...current, id] : current)
  }
  ...
  const lockersWithoutSupply = selectedLockers.filter(lockerId => !scenario.flights.some(pair => pair.lockerId === lockerId && selectedDepots.includes(pair.depotId) && pair.returnDistanceMm <= scenario.defaults.flightLimitMm))
  const supplyPreviewReady = selectedLockers.length > 0 && selectedDepots.length > 0 && lockersWithoutSupply.length === 0
  ...
  const title = [
    'Can this neighbourhood deliver?', 'Build a plan that fits.', 'A good local choice can block the whole plan.',
    'Give Z3 the rules, not a layout.', 'What would make it work?',
  ][chapter]
    ...
    <main className="workspace">
      ...
      <aside className="story-panel" aria-label="Self-guided solver tour">
        ...
        <div className="story-content">
          ...
          {chapter === 0 && <>
            <p className="lead">People need a short walk to a parcel locker. Drones need a workable supply network. Can we choose the locations so every rule is met?</p>
            ...
            <p className="interaction-note"><strong>Step 1 is for inspection.</strong> Select a population cell or an open locker to trace a collection journey. The eight-locker example is fixed here: you choose supply depots in step 2, and Z3 chooses the lockers in step 4.</p>
          </>}
          {chapter === 1 && <>
            <p className="lead">Start with an empty plan. Choose exactly <strong>eight lockers</strong> and <strong>two depots</strong>. Every population cell needs a walk of at most 500 m, and every selected locker needs a supply connection within a 2 km return flight.</p>
            <p className="interaction-note"><strong>You can build on the map.</strong> Click any locker circle or depot label to toggle it; the buttons below provide the same controls. After choosing depots, coral rings expose locker candidates that none of them can reach.</p>
            <p className="game-scale">There are <strong>38,610</strong> possible site combinations before assigning the 33 cells or checking shared capacities.</p>
            <fieldset disabled={busy} className="site-picker locker-picker"><legend>Lockers · {selectedLockers.length}/8 selected</legend>{scenario.lockers.map(locker => <button key={locker.id} aria-label={`Locker ${locker.id}`} aria-pressed={selectedLockers.includes(locker.id)} disabled={!selectedLockers.includes(locker.id) && selectedLockers.length === scenario.defaults.maxLockers} onClick={() => toggleLocker(locker.id)}>{locker.id}</button>)}</fieldset>
            <fieldset disabled={busy} className="site-picker depot-picker"><legend>Depots · {selectedDepots.length}/2 selected</legend>{scenario.depots.map(depot => <button key={depot.id} aria-pressed={selectedDepots.includes(depot.id)} disabled={!selectedDepots.includes(depot.id) && selectedDepots.length === scenario.defaults.maxDepots} onClick={() => toggleDepot(depot.id)}>◇ {depot.id}</button>)}</fieldset>
            <div className="game-checks" aria-label="Quick map checks" aria-live="polite">
              ...
              <p className={supplyPreviewReady ? 'pass' : 'warning'}><span>{supplyPreviewReady ? '✓' : '!'}</span><strong>{!selectedLockers.length ? 'Choose lockers to check supply reach' : !selectedDepots.length ? 'Choose depots to preview direct supply reach' : lockersWithoutSupply.length ? `${lockersWithoutSupply.length} selected lockers have no chosen depot in range` : 'Every selected locker has a chosen depot in range'}</strong></p>
              <p className={siteCountsReady ? 'pass' : 'warning'}><span>{siteCountsReady ? '✓' : '!'}</span><strong>{siteCountsReady ? 'Site budgets filled' : 'Select exactly 8 lockers and 2 depots'}</strong></p>
            </div>
            <p className="quiet">These quick checks show obvious geographic problems. They do not assign parcels or prove that locker and shared depot capacities work.</p>
            <button className="primary" disabled={!ready || !siteCountsReady} onClick={() => solve('Your eight lockers and two depots', 'selection', { fixedLockerIds: selectedLockers, fixedDepotIds: selectedDepots })}>Check my plan with Z3 <Cpu size={18} /></button>
          ...
          {chapter === 2 && <>
            <p className="lead">The collection plan looks reasonable. Its supply connections require three different parts of the map.</p>
            <div className="conflict-chain"><p><b>Locker H</b><span>can only use</span><strong>South</strong></p><p><b>Locker I</b><span>can only use</span><strong>West</strong></p><p><b>Locker E</b><span>needs either</span><strong>East or North</strong></p></div>
            <p>That is <strong>at least three depots</strong>, even before considering supply capacity. No choice of two can supply these eight lockers under the 2 km return-flight rule.</p>
            <button className="primary" disabled={!ready} onClick={() => solve('Any two depots, starter lockers fixed', 'starter', { fixedLockerIds: scenario.starterLockerIds })}>Ask Z3 to check every choice <Cpu size={18} /></button>
            <p className="quiet">“Every choice” is expressed as a constraint problem. Z3 does not need a hand-written loop through every layout.</p>
          </>}
          {chapter === 3 && <>
            <p className="lead">Keep the same rules. Let the solver choose the lockers, depots and parcel assignments together.</p>
            <RuleCard limits={scenario.defaults} />
            ...
            <button className="primary" disabled={!ready} onClick={() => solve('Joint locker and depot choices', 'all')}>Find a connected network <Cpu size={18} /></button>
            ...
            <section className="solver-comparison-intro" aria-labelledby="compare-title"><h2 id="compare-title">How does Z3 compare?</h2><p>Run the same question through Z3 and a transparent JavaScript brute-force search. Try both finding a plan and proving that no seven-locker plan exists.</p><label>Question to compare<select aria-label="Question to compare" value={comparisonCase} onChange={event => { setComparisonCase(event.target.value as 'feasible' | 'impossible'); setComparison(undefined) }}><option value="feasible">Find any network</option><option value="impossible">Can seven lockers work?</option></select></label><button disabled={!ready || exhaustiveEngine.phase !== 'ready'} onClick={compareSolvers}>Compare both search methods</button>{exhaustiveEngine.phase === 'error' && <p role="alert">The brute-force worker could not start: {exhaustiveEngine.message}</p>}<p className="quiet">Both receive identical rules and use the same independent result checker. They run one after the other so they do not compete for your CPU. Initialization is excluded. One browser run is an illustration, not a general benchmark.</p></section>
            {comparison && <ComparisonPanel comparison={comparison} />}
          </>}
          {chapter === 4 && <>
            <p className="lead">Different changes repair different conflicts. The walking limit stays at 500 m in every experiment.</p>
            <div className="experiments">
              <button disabled={!ready} onClick={() => solve('Starter lockers, one extra depot', 'starter', { fixedLockerIds: scenario.starterLockerIds, maxDepots: 3 })}><strong>Keep the lockers. Add a depot.</strong><span>Starter layout · up to 3 depots · 2 km return flight</span><ArrowRight size={18} /></button>
              <button disabled={!ready} onClick={() => solve('Starter lockers, longer drone range', 'starter', { fixedLockerIds: scenario.starterLockerIds, flightLimitMm: 2400000 })}><strong>Keep the lockers. Extend drone range.</strong><span>Starter layout · 2 depots · 2.4 km return flight</span><ArrowRight size={18} /></button>
              <button disabled={!ready} onClick={() => solve('At most seven lockers, all sites free', 'all', { maxLockers: 7 })}><strong>Can we manage with seven lockers?</strong><span>All locations free · original supply rules</span><ArrowRight size={18} /></button>
            </div>
            <details className="custom-rules"><summary>Try your own combination</summary><p>All site choices are free here. Capacity is shared, and each cell’s demand stays together.</p><fieldset disabled={busy}>
              <label>Locker budget <input type="number" min={0} max={15} value={limits.maxLockers} onChange={e => { setLimits({ ...limits, maxLockers: Number(e.target.value) }); setRun(undefined) }} /></label>
              <label>Depot budget <input type="number" min={0} max={4} value={limits.maxDepots} onChange={e => { setLimits({ ...limits, maxDepots: Number(e.target.value) }); setRun(undefined) }} /></label>
          ...
          {chapter > 0 && chapter < 4 && <button className="next-button" onClick={() => go(chapter + 1)}>{['', 'Reveal the conflict', 'Let Z3 choose the whole network', 'Explore the trade-offs'][chapter]} <ArrowRight size={17} /></button>}
...
function RuleCard({ limits }: { limits: Limits }) {
  return <div className="rule-card"><div><strong>500 m</strong><span>maximum walk along paths</span></div><div><strong>{limits.maxLockers} + {limits.maxDepots}</strong><span>lockers + depots, at most</span></div><div><strong>{limits.flightLimitMm / 1000000} km</strong><span>maximum return flight</span></div><div><strong>{limits.lockerCapacity} / {limits.depotCapacity}</strong><span>parcels per locker / depot daily</span></div></div>
}
...
function ComparisonPanel({ comparison }: { comparison: Comparison }) {
  ...
  return <section className="solver-comparison" aria-label="Solver performance comparison" aria-live="polite" data-testid="solver-comparison">
    ...
    {finished && <p className={agrees ? 'comparison-agrees' : 'comparison-disagrees'}>{agrees ? `Both methods reached the same ${comparison.z3!.status === 'feasible' ? 'feasible' : comparison.z3!.status.toUpperCase()} conclusion.` : 'The methods disagree. Treat this as an implementation error, not a planning result.'}</p>}
...
function Result({ result, run }: { result: SolveResult; run: Run }) {
  return <section className={`result ${result.status}`} aria-label="Solver result" aria-live="polite" data-testid="solver-result" data-status={result.status}>
    ...
    {result.status === 'feasible' ? <>
      ...
      <p className="verification">Z3 found the hidden assignments; separate JavaScript verified {result.verification.routesChecked} walks ≤ 500 m, all return flights ≤ {run.request.flightLimitMm / 1000000} km, and every daily capacity.</p>
      ...
      <p>{run.scope === 'selection' ? 'This exact site selection does not work. That alone says nothing about other selections.' : run.scope === 'starter' ? 'No assignment works while keeping exactly the starter lockers and the stated limits.' : 'No combination of the 15 candidate lockers and 4 candidate depots works under these limits.'}</p>
    ...
    <p className="result-receipt">Limits: {run.request.maxLockers} lockers · {run.request.maxDepots} depots · {run.request.flightLimitMm / 1000000} km return · {run.request.lockerCapacity}/{run.request.depotCapacity} parcels per day. {result.elapsedMs} ms including validation.</p>
  </section>
}
...
function MethodDialog({ scenario, onClose }: { scenario: Scenario; onClose: () => void }) {
  ...
  return <dialog ref={dialog} onCancel={onClose} className="method-dialog"><button className="dialog-close" onClick={onClose} autoFocus>Close</button><span className="eyebrow">BEHIND THE TOUR</span><h2>What does Z3 actually do?</h2>
    ...
    <div className="method-step"><Box /><div><h3>2. The model describes the choices.</h3><p>Open this locker? Assign this cell to it? Supply it from this depot? Each is a yes/no variable. Counts and parcel totals must respect the limits.</p></div></div>
    ...
    <div className="method-step"><CheckCircle2 /><div><h3>4. Separate code checks the answer.</h3><p>It reconstructs every chosen walking route, recomputes flight distances and counts every parcel at both the locker and depot.</p></div></div>
    <h3>Why use a solver here?</h3><p>A coverage map can show who could reach a locker. It cannot by itself choose a capacity-feasible supply network. Locker choice changes both collection and supply possibilities. Z3 handles their interaction in one model. This is not a claim that Z3 is the only tool: integer programming or custom search can model this problem too.</p>
