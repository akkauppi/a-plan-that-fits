      ...
      <aside className="story-panel" aria-label="Self-guided solver tour">
        ...
        <div className="story-content">
          ...
          {chapter === 3 && <>
            ...
            <section className="solver-comparison-intro" aria-labelledby="compare-title"><h2 id="compare-title">How does Z3 compare?</h2><p>Run the identical question through Z3 and transparent JavaScript enumeration. Finding an early solution can be easy; proving every three-depot choice impossible requires exhausting the remaining branches.</p><label>Question to compare<select aria-label="Question to compare" value={comparisonCase} onChange={event => { setComparisonCase(event.target.value as 'feasible' | 'impossible'); setComparison(undefined) }}><option value="feasible">Find any resilient network</option><option value="impossible">Can three depots survive an outage?</option></select></label><button disabled={!ready || exhaustiveEngine.phase !== 'ready'} onClick={compareSolvers}>Compare both search methods</button>{exhaustiveEngine.phase === 'error' && <p role="alert">The brute-force worker could not start: {exhaustiveEngine.message}</p>}<p className="quiet">Both receive identical rules and use the same independent result checker. They run one after the other so they do not compete for your CPU. Initialization is excluded. One browser run is an illustration, not a general benchmark.</p></section>
