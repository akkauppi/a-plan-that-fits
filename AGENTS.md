...
- Assign every cell's whole illustrative daily demand to exactly one open locker. Assign each open locker to exactly one open depot. Both capacities apply, with depot load summed across its lockers. No unused open sites.
...
- The planning game must remain winnable: exactly 8 lockers and 2 depots have a tested feasible selection. Live game checks show necessary geographic conditions only; they must not claim full feasibility before Z3 and the independent verifier run. Keep an immediate “Let Z3 find a plan” escape hatch.
- In the game, map locker circles and depot labels are controls. A locker’s depot-reach warning must be derived from `scenario.flights` and the active return-flight limit. Do not replace this with a geometric map radius: flight eligibility uses the frozen exact return distances.
