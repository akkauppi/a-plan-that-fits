...
| `recipe.json` | Fixed candidate node IDs, starter locker set and teaching defaults |
...
The snapshot identifier hashes the generated content, including provenance and recipe-derived choices. It is not a claim that the data reflects present-day conditions. Baseline snapshot: `parcel-cabba165e255063e4d7264ef`.
...
The 15 hypothetical locker candidates are **not existing parcel lockers**, a random scatter, or Z3's output. They were generated by this simple geographic procedure:
...
3. Choose the node that adds an option for the largest number of cells still having fewer than **two**. Skip nodes less than **80 m straight-line distance** from an already shortlisted site.
...
5. Repeat until every cell has at least two candidate collection sites. It takes **15 additions**, labelled A–O in selection order.

This is a greedy coverage heuristic: a rule that makes one locally useful addition at a time. It is **not** a globally optimal shortlist or a joint capacity/supply solution. The two-option target creates choices for the lesson, not a requirement that both lockers open. Every cell is later assigned to exactly one locker. The 80 m spacing is purely a candidate-diversity rule; it does not replace the 500 m walking rule.
...
Four depot candidates were hand-picked in different parts of the map to create geographically separated supply choices. The prepared eight-locker subset was chosen to demonstrate a walking-feasible layout with a supply conflict. Neither the depot choices nor the starter subset are outputs of the shortlist heuristic. Capacity assignments for the starter are then built deterministically as documented in [the model contract](../docs/model.md).
